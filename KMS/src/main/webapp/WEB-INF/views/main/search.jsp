...
$(document).ready(function(){
	...
	$(this).attr("title", "리스트 | 통합 검색 결과 | M-Library");
	
	$('#gSearchType').val('<c:out value="${params.searchType}"/>');
	$('#gSearchType').msDropDown().data("dd").set("value", '<c:out value="${params.searchType}"/>');
	
	$('#gSearchText').val('<c:out value="${params.searchText}"/>');	
});
